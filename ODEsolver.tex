...
\medskip
\begin{tcolorbox}[colframe=black!15!white, coltitle=white!5!black, title = \bf One Step Method (One)]
Let $t_0=a,\, u_0 = y_0$. For $n=0,1,2,...,$
\begin{align*}
t_{n+1} &= t_n + h_n,\\
u_{n+1} &= u_n + h_n \Phi(t_n, u_n; h_n).
...
$$
Introduce the vector space $\Gamma_h = \{ \bs v = (v_0, v_1, \ldots, v_{N})\}\cong \mathbb R^{N+1}$ equipped with $\|\cdot\|_{\infty}$ norm. 

For $v\in C^1[a,b]$, define the residual operator $R v=v^{\prime}(t)-f(t, v(t))$. Similarly, define the discrete residual operator
$$
\left(R_h \bs v\right)_n =\frac{1}{h}\left(v_{n+1}-v_n\right)-\Phi(t_n, v_n; h), \quad n =0,1,\ldots, N-1.
$$
...
\begin{theorem}
If ${\Phi}(t, \boldsymbol{y} ; h)$ satisfies a Lipschitz condition with respect to the $y$-variables,
$$
\left\|{\Phi}(t, \boldsymbol{y} ; h)-{\Phi}\left(x, \boldsymbol{y}^* ; h\right)\right\| \leq M\left\|\boldsymbol{y}-\boldsymbol{y}^*\right\| \text { on }[a, b] \times \mathbb{R}^d \times\left[0, h_0\right]
$$
then the One Step Method (One) is stable.
\end{theorem}
...
Recall the definition of truncation error
\begin{equation}\label{eq:Tphi}  
T(t, y; h) = \frac{1}{h}(u_{\rm next} - y_{\rm next}) =  \Phi(t, y; h) - \frac{1}{h} \int_{t}^{t+h} f(s, y(s)) \, \dd s.  
\end{equation}  
